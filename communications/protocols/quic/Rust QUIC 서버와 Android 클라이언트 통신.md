...
### 2.1  라이브러리 선택: `quinn` vs. `s2n-quic`
...
## 3. 부: Android QUIC 클라이언트: 두 가지 경로와 올바른 선택
...
### 3.1  경로 A: 표준 접근법, Cronet (그리고 그 한계)
...
### 3.2  경로 B: 유연하고 강력한 접근법, Rust 네이티브 라이브러리 (JNI)
...
### 5.1  Rust QUIC 서버 (`server/src/main.rs`)
...
### 5.2  Rust JNI 클라이언트 라이브러리 (`android/rust-client/src/lib.rs`)
...
### 5.3  Kotlin JNI 래퍼 클래스 (`QuicClient.kt`)
...
### 5.4  Android UI 및 클라이언트 로직 (`MainActivity.kt`)
