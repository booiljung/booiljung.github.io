...
#### 2.1.1  네트워크 모델에서의 전송 계층 역할

##### 2.1.1.1  TCP/IP 모델과 OSI 모델
...
#### 2.1.3  TCP 대 UDP: 근본적인 트레이드오프 분석
...
#### 2.2.2  캡슐화: 애플리케이션 메시지에서 네트워크 프레임까지의 여정
...
#### 3.2.1  순서 번호(Sequence Number)와 확인 응답 번호(Acknowledgment Number)의 역할
...
#### 4.2.2  TCP BBR: 모델 기반 제어로의 패러다임 전환
...
### 5.1  네트워크 병목 현상과 TCP에 미치는 영향

#### 5.1.1  버퍼블로트: 과도한 버퍼링의 위험
...
#### 5.1.2  HOL 블로킹 (Head-of-Line Blocking)
...
#### 5.3.1  `sysctl`을 이용한 리눅스 TCP 스택 최적화
...
#### 6.1.2  아키텍처 비교: QUIC over UDP 대 TCP
...
#### 6.2.1  다중 경로 TCP (MPTCP): 아키텍처와 이점
