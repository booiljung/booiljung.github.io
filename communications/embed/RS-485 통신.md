...
## 2.  핵심 원리: 차동 신호 방식 (Core Principle: Differential Signaling)
...
### 2.2  공통 모드 노이즈 제거(Common-Mode Rejection)
...
### 2.3  장거리 전송 및 접지 루프(Ground Loop) 내성
...
### 3.2  공통 모드 전압 범위(Common-Mode Voltage Range)
...
### 4.1  권장 토폴로지: 데이지 체인(Daisy Chain)
...
### 4.3  반이중(Half-Duplex) 및 전이중(Full-Duplex) 통신
...
### 5.1  종단 저항(Termination Resistor)
...
### 5.2  페일세이프 바이어싱(Failsafe Biasing)
...
### 5.3  케이블링, 접지, 차폐(Cabling, Grounding, Shielding)
...
### 6.1 RS-485 vs. RS-232/RS-422: 다중점 통신의 혁신
...
### 6.2 RS-485 vs. 이더넷(Ethernet): 복잡성, 비용, 환경적 강점 비교
...
### 6.3 RS-485 vs. CAN 버스(CAN Bus): 프로토콜 내장 여부와 중재 방식의 차이
...
### 7.1  산업 자동화 (Industrial Automation)
...
### 7.2  빌딩 자동화 및 HVAC (Building Automation and HVAC)
...
### 7.3  원격 측정 및 모니터링 (Remote Telemetry and Monitoring)
