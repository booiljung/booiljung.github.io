...
### 0.2 B. 산업용 필드버스의 한계와 통합 네트워크의 부상
...
## 1. II. TSN의 핵심 기술: IEEE 802.1 표준 상세 분석
...
### 1.1 A. 시간 동기화 (Time Synchronization): IEEE 802.1AS (gPTP)
...
### 1.2 B. 트래픽 스케줄링 및 셰이핑 (Traffic Scheduling and Shaping)

시간 동기화가 완료되면, TSN은 이 시간 정보를 기반으로 다양한 트래픽의 전송 시점을 제어하고 흐름을 조절한다. 이를 위해 여러 스케줄링 및 셰이핑 표준이 사용된다.

#### 1.2.1  IEEE 802.1Qbv - 시간 인식 셰이퍼 (Time-Aware Shaper, TAS)
...
#### 1.2.2  IEEE 802.1Qbu / 802.3br - 프레임 선점 (Frame Preemption)
...
#### 1.2.3  IEEE 802.1Qav - 크레딧 기반 셰이퍼 (Credit-Based Shaper, CBS)
...
### 1.3 C. 신뢰성 및 장애 허용 (Reliability and Fault Tolerance): IEEE 802.1CB (FRER)
...
### 4.2 B. TSN의 미래: 5G와의 융합과 차세대 애플리케이션
