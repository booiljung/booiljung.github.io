...
### 1.1  오픈 소스 표준의 기원: 타카스 토모지(Tomoji Takasu)의 개발
...
### 1.2  핵심 철학: 이식 가능한 프로그램 라이브러리와 애플리케이션 스위트
...
### 1.3  라이선스와 커뮤니티 영향: BSD 라이선스의 의미
...
### 2.1  소프트웨어 아키텍처: ANSI C 기반의 심층 분석
...
### 2.3  측위 모드의 분류: 단독 측위부터 PPP-Fixed까지
...
#### 3.2.3  명령줄 인터페이스를 이용한 워크플로우 자동화 (예: RNX2RTKP)
...
### 3.3  RTKPLOT: GNSS 데이터 시각화 및 분석

#### 3.3.1  측위 결과 및 지상 궤적 플로팅
...
### 3.4  유틸리티 애플리케이션: RTKCONV, STRSVR 등
...
## 4.  현대 RTKLIB 생태계: 포크, 래퍼 및 커뮤니티 개발
...
### 4.3  Python으로 기능 확장: `pyrtklib`와 `rtklib-py` 분석
...
### 5.2  무인 항공기(UAV) 사진측량: 항공 매핑에서 RTK/PPK의 역할
...
### 6.1  RTKLIB 대 상용 소프트웨어 (Trimble Business Center, GrafNav)
