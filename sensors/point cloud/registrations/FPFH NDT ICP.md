...
## 1. 포인트 클라우드 정합의 필요성과 다단계 접근법

### 1.1 포인트 클라우드 정합의 정의와 최종 목표
...
## 2. 단계 (Coarse): FPFH와 RANSAC을 이용한 대략적 초기 정렬
...
### 2.1 FPFH (Fast Point Feature Histogram)의 원리
...
#### 2.2.2 Step 2: SPFH (Simplified Point Feature Histogram) 계산
...
#### 2.2.3 Step 3: 이웃 가중치를 통한 FPFH 생성
...
### 2.3 RANSAC을 통한 강인한 정합 (Robust Alignment via RANSAC)
...
### 3.1 NDT (Normal Distributions Transform) 심층 분석
...
### 3.2 ICP (Iterative Closest Point) 심층 분석
...
## 4. 통합 파이프라인: FPFH + NDT + ICP 결합 전략 및 최적화
...
### Table 2: 주요 파이프라인 파라미터 요약 및 튜닝 가이드
...
### 5.1 FPFH+NDT+ICP 파이프라인의 종합적 평가
