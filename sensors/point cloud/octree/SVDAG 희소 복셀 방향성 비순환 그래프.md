...
## 2.  기반 기술 - 희소 복셀 옥트리(SVO)의 원리와 구조
...
### 2.2 SVO(Sparse Voxel Octree)의 개념
...
### 2.3 Laine & Karras의 'Efficient Sparse Voxel Octrees' 심층 분석
...
### 3.2 핵심 원리: 동일 서브트리 병합 (Common Subtree Merging)
...
### 3.3 알고리즘 심층 분석: 상향식(Bottom-up) SVDAG 구축
...
### 4.1 압축 해제 없는 직접 순회 (Direct Traversal)
...
### 4.3 핵심 연산: 레이-AABB(축 정렬 경계 상자) 교차 판정
...
### 5.1 SSVDAG (Symmetry-aware SVDAG)
...
#### 5.4.1 핵심 테이블 1: SVDAG 및 주요 변종 기술 비교
...
### 6.3 기하학을 넘어서: 속성 데이터(Attributes) 압축
...
### 7.2 SVDAG vs. 폴리곤 메시 (Polygon Mesh)
...
#### 7.4.1 핵심 테이블 2: 주요 3D 지오메트리 표현 기술 비교
