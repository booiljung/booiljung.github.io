# CVTNet (LiDAR 기반 위치 인식을 위한 크로스뷰 트랜스포머 네트워크)

## 1.  연구 배경 및 기술적 중요성

### 1.1 A. 질의 명확화 및 보고서 범위 정의
...
### 1.2 B. LiDAR 기반 장소 인식(LPR)의 정의 및 필요성
...
### 1.3 C. 기존 LPR 기술의 한계와 CVTNet의 문제 제기
...
### 1.4 D. 크로스뷰 지리적 위치 추정(CVGL)과의 비교
...
## 2.  LiDAR 데이터 표현 및 처리: CVTNet의 접근 방식





### 2.1 A. 3D 포인트 클라우드를 2D 이미지로 변환
...
### 2.2 B. Range Image View (RIV) 심층 분석
...
### 2.3 C. Bird's Eye View (BEV) 심층 분석
...
### 2.4 D. RIV와 BEV의 상보성 및 요(Yaw) 각도 불변성의 핵심 원리
...
### 3.2 B. Intra-Transformer: 단일 뷰 내의 컨텍스트 모델링
...
### 3.3 C. Inter-Transformer: 뷰 간의 상호 관계 및 융합
...
## 4.  트랜스포머 기술의 이해: Deformable Transformer와 CVTNet의 관계





### 4.1 A. Deformable Transformer (Deformable DETR) 심층 분석
...
### 4.2 B. CVTNet의 트랜스포머와 Deformable Transformer의 비교
...
### 5.4 D. Ablation Study: 핵심 모듈의 기여도 분석
