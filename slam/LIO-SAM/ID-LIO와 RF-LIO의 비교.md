...
## 1.  동적 환경 LIO의 도전과 공통 기반 LIO-SAM

### 1.1  문제 정의: 동적 환경에서의 SLAM
...
## 2.  RF-LIO: '선-제거(Removal-First)' 패러다임의 심층 분석

### 2.1  핵심 철학: "제거 후 정합 (Remove, then Match)"
...
### 2.3  동적 객체 탐지 메커니즘: Removert 알고리즘
...
#### 2.3.1  Range Image 기반 불일치 계산 (Discrepancy Calculation)
...
## 3.  ID-LIO: '인덱스 포인트와 지연된 제거' 전략 탐구

### 3.1  핵심 철학: "최적화 내 통합 관리 (Integrated Management within Optimization)"
...
### 3.3  동적 객체 관리 메커니즘: 다단계 전략

#### 3.3.1  공간적 탐지: Pseudo Occupancy Grid
...
#### 3.3.2  시간적 전파: '인덱스 포인트(Indexed Point)'
...
#### 3.3.3  최적화 통합: '동적 가중치(Dynamic Weights)'
...
#### 3.3.4  맵 관리: '지연된 제거(Delayed Removal)'
...
## 4.  핵심 전략 비교 분석: RF-LIO 대 ID-LIO

### 4.1  패러다임 비교: 선제적 필터링 vs. 통합적 관리
...
### 6.1  적용 분야 및 연구 목적에 따른 선택 가이드라인
