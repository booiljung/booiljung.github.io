...
## 1.  동적 환경 SLAM의 도전 과제와 DRR-LIO의 등장

### 1.1 LiDAR 기반 SLAM의 정적 세계 가정과 그 한계
...
### 1.2 동적 객체가 위치 추정 및 지도 작성에 미치는 영향
...
### 1.3 LiDAR-Inertial Odometry(LIO)의 부상과 DRR-LIO의 연구 동기
...
### 2.1  LIO-SAM의 아키텍처: Tightly-coupled 접근법
...
#### 2.2.3 비용 함수(Cost Function)와 최적화 문제 해석
...
### 2.3  LIO-SAM의 강점과 동적 환경에서의 내재적 한계
...
### 3.2  동적/정적 포인트 식별: 수직 복셀 높이 디스크립터 (Vertical Voxel Height Descriptor)
...
### 4.1  평가 데이터셋: UrbanLoco 및 자체 데이터셋 특성 분석

DRR-LIO의 강건성과 정확성은 동적 요소가 풍부한 실제 도시 환경을 모사하는 데이터셋을 통해 평가되었다.3

#### 4.1.1 공개 데이터셋 (Public Dataset): UrbanLoco
...
#### 4.1.2 자체 수집 데이터셋 (Self-collected Dataset)
...
#### 4.2.2 UrbanLoco 데이터셋 기반 성능 비교 (추정)
...
### 5.1  DRR-LIO vs. LIO-SAM: 동적 환경 대응 능력의 진화
...
### 5.2  기하학적 접근법 비교: DRR-LIO vs. Removert
...
### 5.3  시맨틱 접근법 비교: DRR-LIO vs. SuMa++
...
### 6.1  한계점 분석: 알고리즘의 잠재적 취약점 및 재현성 문제
