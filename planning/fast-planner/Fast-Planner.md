...
## 1. Fast-Planner의 의의와 핵심 기여

### 1.1 배경: 미지 환경에서의 고속 자율 비행의 난제
...
## 2.  Fast-Planner의 아키텍처 및 핵심 구성 요소

### 2.1  계층적 계획 파이프라인 (Hierarchical Planning Pipeline)
...
### 2.2  코드베이스 구조 분석 (GitHub Repository)
...
## 3.  핵심 알고리즘 심층 분석: 이론과 실제

### 3.1  프론트엔드: Kinodynamic Path Searching
...
#### 3.1.1 이론적 기반: 하이브리드 A-star (Hybrid A-star)
...
#### 3.2.3 핵심 최적화 기법: 볼록 포(Convex Hull) 속성 활용
...
### 3.3  실시간 재계획: 위상 경로 탐색과 경로 안내 최적화(PGO)
...
#### 3.3.1 문제점: GTO의 지역 최솟값(Local Minima)
...
### 4.1  Fast-Planner vs. EGO-Planner: ESDF의 유무
...
### 5.1  자율 탐사 (Autonomous Exploration)
