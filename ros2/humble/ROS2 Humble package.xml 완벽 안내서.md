...
### 3.2  그룹 의존성: `<member_of_group>`과 `<group_depend>`
...
### 4.2  `ament_python` Python 패키지 예제
...
## 5.  모범 사례(Best Practices) 및 흔한 실수
...
### 6.1  `rosdep install` 실패: "Cannot locate rosdep definition for [key_name]"
...
### 6.2  `colcon build` 실패: "Package 'package_name' not found"
...
### 6.3  `colcon build` 실패: CMake/Python 관련 오류
