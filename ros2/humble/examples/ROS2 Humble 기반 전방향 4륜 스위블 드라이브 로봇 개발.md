...
## 1. 전방향 이동 로봇과 스위블 드라이브의 이해

### 1.1 홀로노믹(Holonomic) 구동계의 개념과 로보틱스에서의 중요성
...
### 1.2 스위블 드라이브(Swerve Drive)의 작동 원리 및 특성
...
### 1.3 ROS2 Humble 및 `ros2_control` 프레임워크의 역할
...
### 2.1 모듈 구조 설계: 동축(Coaxial) 방식과 비동축 방식

스위블 모듈의 구동 모터와 조향 모터를 배치하는 방식에 따라 크게 동축 방식과 비동축 방식으로 나눌 수 있다.

#### 2.1.1 동축 설계 (Coaxial Design)
...
#### 2.1.2 비동축 설계 (Non-coaxial Design)
...
### 3.2 역기구학(Inverse Kinematics) 모델 유도
...
### 3.3 순기구학(Forward Kinematics) 모델 유도 (오도메트리)
...
### 4.2 소프트웨어 아키텍처: `ros2_control` 중심의 모듈형 구조
...
## 5.  `ros2_control` 프레임워크를 이용한 제어 시스템 구현
...
### 5.2 하드웨어 인터페이스(Hardware Interface) C++ 플러그인 개발
...
### 5.3 스위블 드라이브 컨트롤러(Swerve Drive Controller) 노드 개발
...
### 6.1 `robot_localization` 패키지를 이용한 EKF 센서 융합
...
### 6.3 기구학적 특이점(Kinematic Singularity) 문제
