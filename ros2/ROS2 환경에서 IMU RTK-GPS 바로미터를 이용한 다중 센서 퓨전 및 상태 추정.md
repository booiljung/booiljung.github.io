# ROS2 환경에서 IMU, RTK-GPS, 바로미터를 이용한 다중 센서 퓨전 및 상태 추정 전문가 보고서
...
### 5.3  `process_noise_covariance`(`Q` 행렬) 튜닝
