...
## 1.  임베디드 로보틱스의 새로운 패러다임, micro-ROS

### 1.1 로봇 운영체제(ROS)의 진화와 리소스 제약적 환경의 과제
...
### 1.2 micro-ROS의 탄생: 개발 목표와 핵심 철학
...
### 1.3 ROS 2 생태계의 공식 확장으로서의 micro-ROS 위상
...
### 2.2 클라이언트-에이전트 모델: 임베디드와 ROS 2 데이터스페이스의 연결
...
### 2.3 micro-ROS 에이전트의 역할: 검색(Discovery) 및 QoS 관리 오프로딩
...
### 2.4 클라이언트 라이브러리: `rcl`과 MCU 최적화를 위한 `rclc` 확장
...
### 2.5 미들웨어(RMW) 계층: eProsima Micro XRCE-DDS 표준의 이해
...
### 3.2 공식 지원 하드웨어 플랫폼: 주요 보드 및 사양 분석
...
## 4.  기술 비교 분석: ROSSerial, MQTT와의 관계
...
### 4.1 레거시 통합 방식과의 비교: ROSserial과의 심층 분석
...
### 4.2 IoT 및 엔터프라이즈 통합: MQTT와의 연동 아키텍처 패턴
...
### 8.1 현재 진행 중인 개발: 임베디드 워킹 그룹의 논의
...
### 8.2 차세대 미들웨어: Zenoh의 등장과 DDS를 넘어선 성능
...
### 8.3 하드웨어 가속과 "ROS 2 on a Chip": 임베디드 로보틱스의 새로운 지평
...
### 9.1 신규 프로젝트에 micro-ROS 도입을 고려하는 개발자를 위한 제언
...
### 9.2 ROS 2와 함께하는 임베디드 로보틱스의 미래에 대한 최종 고찰
