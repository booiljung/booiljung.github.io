...
### 2.1  의존관계 역전 원칙(DIP): '무엇'에 의존할 것인가 (Shape)
...
### 2.2  제어의 역전(IoC): '누가' 제어할 것인가 (Direction)
...
### 2.3  의존성 주입(DI): '어떻게' 관계를 맺을 것인가 (Wiring)
...
### 2.4  종합적 분석: Martin Fowler의 관점을 통한 세 개념의 역할 구분
...
#### 3.1.1  생성자 주입 (Constructor Injection)
...
#### 3.1.2  세터 주입 (Setter/Property Injection)
...
### 3.2  서비스 로케이터(Service Locator) 패턴
...
### 4.1  Spring Framework: Bean 생명주기 관리의 정수
...
### 4.2 .NET Core: 내장 DI 컨테이너와 서비스 생명주기
...
### 5.1 IoC와 관점 지향 프로그래밍(AOP)의 시너지
...
### 5.2 마이크로서비스 아키텍처(MSA)에서의 IoC/DI의 중요성
