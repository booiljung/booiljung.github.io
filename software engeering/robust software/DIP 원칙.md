...
### 1.3 의존성의 소유권 역전 (Inversion of Ownership)
...
## 2.  DIP 구현의 핵심 패턴: 의존성 주입(DI)과 제어의 역전(IoC)
...
### 2.4 테이블 1: 전통적 의존성과 역전된 의존성의 특성 비교
...
## 3.  SOLID 원칙의 시너지: OCP와 LSP를 완성하는 DIP의 역할
...
### 3.1  LSP 위반이 OCP 위반으로 이어지는 과정 분석
...
#### 3.1.1 LSP 위반 시나리오: Rectangle과 Square
...
### 3.3 DIP는 OCP와 LSP의 '구조적 보증 장치'이다
...
## 4.  산업 표준의 구현: Spring 프레임워크 DI 컨테이너 심층 분석
...
### 4.4 테이블 2: Spring 프레임워크의 의존성 주입 방식 비교
...
### 5.1  DIP 적용의 실용적 가이드라인: 언제 적용할 것인가?
...
### 5.3 DIP는 은총알이 아닌, 비용-편익 분석이 필요한 아키텍처 도구이다
