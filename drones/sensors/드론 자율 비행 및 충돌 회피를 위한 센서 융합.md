# 드론 자율 비행 및 충돌 회피를 위한 LiDAR, IMU, GNSS 융합: 종합 튜토리얼
...
- **절차:**
  ...
  2. **클러스터 생성:** 전체 데이터 시퀀스를 다양한 시간 길이 `τ`를 갖는 여러 개의 클러스터로 나눈다.
  3. **알란 분산 계산:** 각 `τ`에 대해, 클러스터 평균들의 2-표본 분산(two-sample variance)을 계산한다. 이것이 알란 분산 `σ²(τ)`이다.30
  4. **알란 편차 플롯 생성:** 알란 분산의 제곱근인 알란 편차 `σ(τ)`를 `τ`에 대해 log-log 스케일로 플로팅한다. 이 플롯을 '시그마-타우 다이어그램'이라고도 부른다.29
- **노이즈 파라미터 식별:** log-log 스케일의 알란 편차 플롯은 특징적인 기울기를 가진 여러 영역으로 나뉜다. 이 기울기를 통해 주요 노이즈 항을 식별하고 그 계수를 산출할 수 있다 29:
  - **Angle/Velocity Random Walk (N):** 플롯에서 기울기가 **-1/2**인 직선을 찾고, 이 직선을 `τ = 1`초까지 연장했을 때의 `σ(τ)` 값을 읽는다. 이 값은 각각 자이로스코프와 가속도계의 광대역 백색 잡음 계수에 해당한다.
  - **Bias Instability (B):** 플롯에서 기울기가 **0**에 가까운, 즉 가장 평평하고 낮은 부분을 찾는다. 이 지점의 `σ(τ)` 값을 `0.664`로 나누면 바이어스 불안정성 계수를 얻을 수 있다.
  - **Rate Random Walk (K):** 플롯에서 기울기가 **+1/2**인 직선을 찾고, 이 직선을 `τ = 3`초까지 연장했을 때의 `σ(τ)` 값을 읽는다. 이는 바이어스의 랜덤 워크 특성을 나타내는 계수다.
...
#### 1.3.2 RANSAC (RANdom SAmple Consensus) 기반 이상치 탐지
...
#### 2.3.3 업데이트(Update) 단계: 외부 측정을 이용한 오차 보정
