...
## 1. 현대 항공 전자의 딜레마 - 성능 대 인증

### 1.1 비행 제어 컴퓨터(FCC)의 진화: 기계적 연결에서 자율 비행의 두뇌로
...
## 2. 결정론의 초석: 실시간 시스템에 대한 심층 탐구

### 2.1 실시간의 정의: 단순한 속도를 넘어서
...
## 3. 듀얼 커널 패러다임: Xenomai 아키텍처 해부

### 3.1 코발트 코어: 리눅스와 나란히 실행되는 실시간 전담 커널
...
### 3.2 인터럽트 파이프라인(Adeos/I-pipe 및 Dovetail): 지연 시간의 문지기
...
### 3.4 Xenomai 4/EVL로의 진화: 복잡성 길들이기
...
## 5. 인증의 관문: DO-178C와 항공기 감항성 확보의 길

### 5.1 DO-178C의 이해: 항공 전자 소프트웨어의 황금률
...
### 5.2 넘을 수 없는 벽: 표준 리눅스 인증이 불가능한 이유
...
### 5.3 아키텍처적 해결책: ARINC 653과 가상화를 통한 격리
...
### 6.2 제언 1: 비인증 및 저수준 DAL 시스템을 위한 최적의 플랫폼
...
### 6.3 제언 2: 높은 DAL 등급 인증 시스템을 위한 게스트 OS 전략
